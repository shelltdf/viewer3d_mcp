#!/usr/bin/env python3
"""
启动 model-processor 的 Vite 开发服务器，并可选在浏览器中打开。

用法（在 model-processor 目录下）：
  python run_web.py
  python run_web.py --port 5173
...
def main() -> int:
    ap = argparse.ArgumentParser(description="model-processor：启动 Vite 开发服务器")
    ...
    if not _VITE_LOCAL.is_file():
        print(
            "未找到本地 Vite（缺少 node_modules）。请先在 model-processor 目录执行：npm install",
            file=sys.stderr,
        )
        return 1
