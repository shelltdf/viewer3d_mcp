#!/usr/bin/env python3
"""
启动 plant-studio 的 Vite 开发服务器，并可选在浏览器中打开。

用法（在 plant-studio 目录下）：
  python run_web.py
...
def main() -> int:
    ap = argparse.ArgumentParser(description="plant-studio：启动 Vite 开发服务器")
    ...
    if not _VITE_LOCAL.is_file():
        print(
            "未找到本地 Vite（缺少 node_modules）。请先在 plant-studio 目录执行：npm install",
            file=sys.stderr,
        )
        return 1
