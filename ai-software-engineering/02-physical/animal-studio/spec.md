...
- **视口**：`CreatureViewport.vue`：`OrbitControls`、简易棚灯、圆地面、**导出 ZIP**（`jszip` + `GLTFExporter.parseAsync`）；烘焙逻辑见 `bakeExport.js`。卸载/重建时释放网格与线段的 `geometry`/`material`（含骨骼可视化）。**角色显示（统一数据源）**：`params.showCreatureModel` / `showSkeleton` / `showCreaturePhysics` 在 **Dock 外观** 与 **视口右上角「显示」** 绑定同一 reactive，只更新图层不触发 `buildCreature`（`creatureGeometryFingerprint` 扣除上述键）。**布娃娃碰撞盒**：优先由 **`attachRagdollShapeExtentsFromSkin`**（地平校正后）据 **`SkinnedMesh`** 顶点与 **主导蒙皮骨** 聚类、沿骨轴 **OBB** 写入每骨 **`userData.ragdollHalfExtents`**，`createRagdoll` 用作 **Box** 半轴；顶点过少等则回退 **`skeletonJointRadius`**（`0.022×scale`）× 骨名系数。关节球可视化仍用 **`skeletonJointRadius`**。
- **物理布娃娃 / cannon-es（预览）**：依赖 **`cannon-es`**。**物理定义**（`ragdollPhysics.js` 的 **`createRagdoll`**）：在视口 **`buildCreature`** 完成并入场景后 **即建立** — **World**、地面 **Plane（y=0）**、按骨段的 **Box** 刚体（根段 **mass 0**）、父子 **PointToPointConstraint**、以及 **调试线框**（`EdgesGeometry`/`LineSegments`）。**菜单栏「物理布娃娃」** 与 **`v-model:ragdoll-enabled`** 仅控制是否 **物理解算**：开启时每帧 **`world.step`** 并 **`syncBonesFromPhysics`**，程序化根位移与 **`applyCreatureJointAnimation` 暂停**；关闭时每帧照常程序动画，并 **`syncBodiesFromBones`** 使刚体与骨骼对齐、线框跟随。**`buildCreature` 重建**或卸载时 **`dispose`** 约束/体/线框。初始化失败（无 `Armature` 等）时关回 **`ragdoll-enabled`**。
