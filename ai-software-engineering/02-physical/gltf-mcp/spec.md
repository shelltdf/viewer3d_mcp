...
- **视口显示**：左右**各自**六类下拉——实体/线框/**点云（顶点）**（对 Mesh/SkinnedMesh 在其下挂 **`Points` 子节点**，共用几何 `position`；**父 Mesh 保持 `visible=true`**，材质换为不写色深的占位 `MeshBasicMaterial`，避免「父不可见则子级点也不渲染」导致**点云整片消失**；`PointsMaterial` 使用 **屏幕像素 `size`**、关闭距离衰减以便辨认）；**顶点属性**（关 / 三角法线 / 各 `geometry.attributes` 名）、光照（影棚/柔和/平铺）、贴图（采样 / 无光照 / 隐藏槽 /**按材质槽名单独预览 `slot:<key>`**，`envMap` 走 `MeshBasic.envMap`，其余可走 `map`）、材质（原始 / 法线 / UV 格）、骨骼（关 / 骨架 / 权重示意色）；其中**顶点属性**紧接实体类下拉，点云模式下顶点属性着色不生效。选定某属性时，调试材质在顶点着色器把该缓冲的**前三分量**线性映射为 RGB（`position` 用 `fract` 避免大坐标裁切），经 `varying` 在三角形内**平滑插值**到片元。由 `applyViewportViewState` + `applySceneLighting` 组合应用。工具栏可对「处理前 / 处理后」视口格子**单独或同时**开关显示；**每格左上角**展示该侧独立的 **存储粗估**（CPU/VRAM 相关字节与 GL 计数摘要）。
...
- **属性面板（model-processor）**：大纲当前焦点为「处理前」时，面板顶部展示**只读**高对比提示；焦点为「处理后」时展示**可编辑**提示，避免用户对错误一侧产生可写预期。选中 **Mesh / SkinnedMesh** 时提供 **「本 Mesh 材质」**：按材质槽切换**类型**（与 `materialReplace.createMaterialByTypeName` 一致）、改**名称**、编辑**常用数值**（如 `roughness` / `metalness` / `shininess` / `opacity` / `emissiveIntensity` 等，随类型存在与否显示）；「在大纲中打开…」跳转至 `材质` 分组下对应条目以使用**完整参数表、贴图槽与预览球**。材质/类型变更经 `inspectorSceneRevision` 触发 `resolveOutlineItem` 刷新，避免 Three.js 就地改字段后 UI 陈旧。
- **顶点焊接（处理后）**：处理菜单首项「顶点焊接」对「处理后」场景全部 `Mesh` / `SkinnedMesh` 调用 `three/examples/jsm/utils/BufferGeometryUtils.mergeVertices`（默认容差 `1e-4`）；按全部顶点属性生成哈希合并重合角点。交错 `InterleavedBufferAttribute` 的网格跳过。执行后弹出结果对话框，汇总处理网格数、顶点前后总数及**合并掉的顶点数** `{W}`；状态栏同步短提示。
- **单贴图补充 PBR（处理后）**：处理菜单「单贴图 → PBR」调用 `supplementPbrFromSingleMap`（`model-processor/src/utils/inferPbrFromSingleMap.js`）：对 `MeshPhong` / `Lambert` / `Basic` / `Toon` 且**已有 `map`** 的材质先转为 `MeshStandardMaterial`（保留常用贴图槽与颜色等；Phong 的 `shininess` 粗映射为初始 `roughness`），再对 Standard / Physical **补充缺失**的 `roughnessMap`（反照率亮度启发式）与 `normalMap`（亮度 Sobel）；需**索引 + UV** 才生成法线并 `computeTangents`（典型 OBJ 无索引时仍可补粗糙度）。生成贴图带 `userData._pbrInferred`；**不写入** `metalnessMap`。`Texture` 数据槽经 `applyTextureColorSpaceForMaterialSlot` 为 `NoColorSpace`；`ImageBitmap` / `Canvas` 图像源参与可读性检测。**从 `map` 读到 2D Canvas 时不得再用 2D 变换预翻转**（否则与 `CanvasTexture.flipY = map.flipY` 叠加会竖直颠倒）；行序与源图像一致，由 Three 按与 `map` 相同的 `flipY` 上传。
- **网格简化（meshoptimizer，处理后）**：`ModelWorkbench` 弹窗中简化目标为**四选一互斥**，以 `fieldset` + 两列网格展示；`tri_ratio` / `tri_count` / `vert_ratio` / `vert_count`；调用 `simplifyResultMeshes` 时仅传入当前模式对应参数（`applyMeshoptSimplifyToGeometry` 的 `targetMode`）。比例类默认 **0.5**；数量类在**打开弹窗**或**切换模式**时默认约为 `getResultSimplifyStats()` 对当前「处理后」场景汇总的三角面数或顶点数之半（至少 1）。执行过程中显示**全屏进度层**，按网格推进。顶点类目标通过三角预算近似，顶点压缩后按模式校验并可能产生日志提示。若简化器**未能减少三角面**（`tri_no_reduction`），不再叠加「仍高于上限」类提示（`tri_above_cap`），避免重复。
- **渲染与后期（model-processor 双视口）**：**每个 3D 格内**各自一行「渲染与效果」，**左右互不联动**：**管线**、**阴影**、**色调映射**（`toneMapping`：`none` / `linear` / `aces` / `reinhard` / `cineon` / `agx` / `neutral`，映射至 `THREE.*ToneMapping`，由 `applyRendererToneAndPipeline` 写入 `WebGLRenderer` 供 **`OutputPass`** 使用）、**SSAO**（`off` / **`on` 合成** / **`only` 仅输出 SSAO 灰度缓冲** 对应 `SSAOPass.OUTPUT`）。链路等价为 `RenderPass` → `SSAOPass` → `OutputPass`。SSAO 尺度与相机 uniform 同步策略见前。`onResize` 时同步 composer 与 **ResizeObserver**。
- **视口轴辅助**：每格**右上角**更小视口（约为原先约 **一半** 边长）：**纯色 RGB `LineSegments`（顶点色）** X 红 / Y 绿 / Z 蓝；**不清除颜色缓冲**，仅 **清深度** 后叠加绘制，背景与主画面一致（无独立底色）。
...
- **辅助地面（工具栏）**：可选「**辅助地面**」：`DualViewport` 在每侧 `Scene` 中、与根模型并列添加 **`CircleGeometry` 圆片** + **`MeshStandardMaterial`（`#cccccc` 约 80% 灰）**，置于该侧根节点包围盒 **底面略下方**，**仅 `receiveShadow`**，半径随包围盒尺度估算；**若无加载模型**则仍放置 **半径 1** 的默认圆面（中心高度约 `y=-0.002`），便于空场景下观察阴影。关闭或卸载模型时移除并 `dispose` 几何与材质。
